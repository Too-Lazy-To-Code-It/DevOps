    ...
    stages {
        ...
        stage('OWASP Scan') {
            steps {
                ...
                sh '''
                    if [ -f /data/jenkins/nvid/dc.h2.db ]; then rm /data/jenkins/nvid/dc.h2.db; fi
                    if [ -f /data/jenkins/nvid/dc.lock.db ]; then rm /data/jenkins/nvid/dc.lock.db; fi
                    if [ -f /data/jenkins/nvid/dc.trace.db ]; then rm /data/jenkins/nvid/dc.trace.db; fi
                '''
        ...
        stage('SonarQube Analysis(SAST)') {
            steps {
                withCredentials([string(credentialsId: 'Sonarqube-url', variable: 'SONARQUBE_HOST_URL'), string(credentialsId: 'sonarqube', variable: 'SONARQUBE_LOGIN')]) {
                    sh '''
        ...
        stage('Docker Compose') {
            steps {
                echo 'Application on starting and monitoring Docker composing...'
